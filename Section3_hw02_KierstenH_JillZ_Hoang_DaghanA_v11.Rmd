...
```{r}
#model2_Ha <- clm(formula = FY16Giving.Grouped ~ Married + Adv.Deg + Gender + AttendenceEvent  + FY15Giving.Grouped + FY14Giving.Grouped +FY13Giving.Grouped + FY12Giving.Grouped, data =givings, link ="logit")
```
...
From the likelihood ratio test, we find that there is strong evidence that association exists for all variables except for AttendenceEvent and Major.Donor.Level.
...
predicts<-cbind(newData, predict(model2_Ha, newdata=newData, type = 'class'))
